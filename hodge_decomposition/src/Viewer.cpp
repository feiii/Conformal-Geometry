...
void drawUv()
{
    ...
    for (CHodgeDecompositionMesh::MeshFaceIterator fiter(g_domain_mesh); !fiter.end(); ++fiter)
    {
        ...
        for (CHodgeDecompositionMesh::FaceVertexIterator fviter(pF); !fviter.end(); ++fviter)
        {
            ...
            glNormal3d(n[0], n[1], n[2]);
            glTexCoord2d(pV->uv_draw[0], pV->uv_draw[1]);
            glColor3f(rgb[0], rgb[1], rgb[2]);
            // glVertex3d(uv[0], uv[1], 0);
            glVertex3d(pV->uv_draw[0], pV->uv_draw[1], 0);
        }
        glEnd();
    }
}
...
void calc_holo_1_form(const std::string& input_mesh)
{
    ...
    else
    {
        ...
        for(size_t i = 0; i < g_meshes.size(); i++) {
            ...
            g_mapper.random_harmonic_form();
            std::vector<M::CHalfEdge*> h1s;
            for(M::MeshHalfEdgeIterator heIter(pM); !heIter.end(); heIter++) {
                M::CHalfEdge *hf = *heIter;
                h1s.push_back(hf);
            }

            for(size_t k = 0; k < i; ++k) {
                M* pMk = g_meshes[k];
                CWedgeOperator<M> wo(pMk, pM);
                double hki = wo.wedge_star_product();
                
                std::deque<M::CHalfEdge*> hks;
                for(M::MeshHalfEdgeIterator heIter(pMk); !heIter.end(); heIter++) {
                    M::CHalfEdge *hf = *heIter;
                    hks.push_back(hf);
                }

                for(size_t i = 0; i < h1s.size(); ++i) {
                    M::CHalfEdge* h1 = h1s[i];
                    M::CHalfEdge* hk = hks.front();
                    while(h1->source()->id() != hk->source()->id() || h1->target()->id() != hk->target()->id()) {
                        hks.pop_front();
                        hks.push_back(hk);
                        hk = hks.front();
                    }
            ...
            CWedgeOperator<M> wo(pM, pM);
            double hii = (wo.wedge_star_product());
