    ...
    if (dimension == 2)
    {
        for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); fiter++)
        {
            ...
            for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); fhiter++)
            {
                ...
                ph->form() = (pf->form() - pfn->form()) / e->weight();
            }
        }
        return;
    }
    ...
    if (dimension == 1)
    {
        for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); viter++)
        {
            ...
            for(M::VertexEdgeIterator eiter(pv); !eiter.end(); eiter++) {
                M::CEdge* edge = *eiter;
                M::CHalfEdge *he = dynamic_cast<M::CHalfEdge*>(edge->halfedge(0));
                double w = he->form();
                if(he->source() != pv) {
                    w = -w;
                }
                pv->form() -= edge->weight() * w;
            }
        }
        return;
    }
    ...
    for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
    {
        ...
        for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); ++fhiter)
        {
            ...
            if (ps != NULL)
            {
               ...
               A_coefficients.push_back(Eigen::Triplet<double>(nid, fid, w));
               sw += w;
            }
            else // boundary face
            {
...
void MeshLib::CHodgeDecomposition::_remove_coexact_form()
{
    ...
    for (M::MeshFaceIterator fiter(pM); !fiter.end(); fiter++)
    {
        ...
        for (M::FaceHalfedgeIterator fhiter(pf); !fhiter.end(); fhiter++)
        {
            ...
            M::CEdge * e = dynamic_cast<M::CEdge*>(ph->edge());
            if(e->boundary()) {
                ph->form() += (f->form()) / e->weight();
                continue;
            }

            M::CFace* fn = dynamic_cast<M::CFace*>(ph->he_sym()->face());
            
            if(pf == ph->face()) {
                ph->form() += (f->form() - fn->form()) / e->weight();
            } else {
                ph->form() -= (f->form() - fn->form()) / e->weight();
            }
    ...
    CHodgeDecompositionMesh* pDomain)
{
    ...
    while (!vqueue.empty())
    {
        ...
        for (M::VertexEdgeIterator veiter(head); !veiter.end(); veiter++)
        {
            ...
            tail->uv_draw = tail->uv();
           //tail->uv_draw[0] = exp(tail->uv()[0]);
            // tail->uv_draw[0] = exp(tail->uv()[0]) * std::cos(tail->uv()[1]);
            // tail->uv_draw[1] = exp(tail->uv()[0]) * std::sin(tail->uv()[1]);
        }
    }


    MeshLib::CPoint2 point_max(0, 0);
    MeshLib::CPoint2 point_min(10000000000, 10000000000);
    std::unordered_map<int, CVertex*> vertex_map;
    for (M::MeshVertexIterator viter(pDomain); !viter.end(); viter++)
    {
        M::CVertex* v = *viter;
        if(vertex_map.count(v->father())) {

            if(point_max[0] < vertex_map[v->father()]->uv_draw[0]) {
                point_max[0] = vertex_map[v->father()]->uv_draw[0];
            }

            if(point_max[1] < vertex_map[v->father()]->uv_draw[1]) {
                point_max[1] = vertex_map[v->father()]->uv_draw[1];
            }
            
            if(vertex_map[v->father()]->uv_draw[0] < point_min[0]) {
                point_min[0] = vertex_map[v->father()]->uv_draw[0];
            }

            if(vertex_map[v->father()]->uv_draw[1] < point_min[1]) {
                point_min[1] = vertex_map[v->father()]->uv_draw[1];
            }

        } else {
            vertex_map.emplace(v->father(), v);
        }
    }

    CPoint2 scale = point_max - point_min;
    double max_y = 0;
    ...
    {
        M::CVertex* v = *viter;
        v->uv_draw = v->uv_draw - point_min;
        v->uv_draw[0] /= scale[1];
        v->uv_draw[1] /= scale[1];
        if(v->uv_draw[1] > max_y) {
            max_y = v->uv_draw[1];
        }

        if(v->uv_draw[1] < min_y) {
            min_y = v->uv_draw[1];
        }

        v->uv_draw = v->uv_draw * 2 * M_PI;
    }

    vertex_map.clear();
    double scale_true = 0.0;
    for (M::MeshVertexIterator viter(pDomain); !viter.end(); viter++)
    {
        M::CVertex* v = *viter;
        if(vertex_map.count(v->father())) {
            scale_true = std::abs(v->uv_draw[1] - vertex_map[v->father()]->uv_draw[1]);
        } else {
            vertex_map.emplace(v->father(), v);
        }
    }

    scale_true = 2 * M_PI / scale_true;
     for (M::MeshVertexIterator viter(pDomain); !viter.end(); viter++)
    {
        M::CVertex* v = *viter;
        v->uv_draw *= scale_true;
    
        double r = exp(v->uv_draw[0]);
        double theta = v->uv_draw[1];
        v->uv_draw[0] = r * std::cos(theta);
        v->uv_draw[1] = r * std::sin(theta);
    }
